...
PROJECT="${1:?usage: scripts/local-build-project.sh <project>}"
...
PROJECT_WORK_DIR="$LOCAL_WORK_ROOT/$PROJECT"
export PROJECT_SOURCE_DIR="$PROJECT_WORK_DIR/source"
export PROJECT_BUILD_DIR="$PROJECT_WORK_DIR"
export PROJECT_OUTPUT_DIR="$LOCAL_OUTPUT_ROOT/$PROJECT"
...
if ((${#debs[@]} == 0)); then
  echo "No .deb packages were produced for $PROJECT"
  exit 1
fi
